// creating an array and passing the number, questions, options, and answers
let questions = [
    {
    numb: 1,
    question: "Las prácticas acrobáticas en el suelo se afianzaron en numerosos pueblos",
    img: "assets/img/Historia.jpg",
    answer: "A lo largo de Europa, Asia y África",
    options: [
        "A lo largo de Europa, Asia y África",
        "A lo largo de America, Asia y África",
        "A  lo largo de Europa y America",
        "A  lo largo de Europa"
    ]
  },
    {
    numb: 2,
    question: "Los inicios de la gimnasia en México fueron principalmente",
    "img": "assets/img/Uni1/acrobatas_aztecas_unegimnasia.jpg",
    answer: "Durante los s. XVIII y XIX",
    options: [
      "Durante los s. XVI",
      "Durante los s. XVI y XIX",
      "Durante los s. XVII y XVIII",
      "Durante los s. XVIII y XIX"
    ]
  },
    {
    numb: 3,
    question: "La gimnasia artística varonil mexicana debutó en los Juegos Centroamericanos y del Caribe a partir de la ",
    "img": "assets/img/Uni1/mexicana_debutó.JPG",
    answer: "Quinta edición",
    options: [
      "Sexta  edición ",
      "Cuarta edición",
      "Primera edicion",
      "Quinta edición"
    ]
  },
    {
    numb: 4,
    question: "¿cuántas medallas obtuvo mexico en los juegos centroamericanos? ",
    "img": "assets/img/Uni1/1532348458_445118_1532401623_noticia_normal.jpg",
    answer: "Nueve  medallas individuales y una más por equipos",
    options: [
      "Diez medallas individuales y una más por equipos",
      "Nueve  medallas individuales y una más por equipos",
      "Nueve  medallas individuales y dos más por equipos",
      "Nueve  medallas individuales y tres más por equipos"
    ]
  },
    {
    numb: 5,
    question: "La gimnasia artística varonil mexicana debutó en los Juegos Centroamericanos y del Caribe a partir de la ",
    "img": "assets/img/Uni1/mexicana_debutó.JPG",
    answer: "Quinta edición",
    options: [
      "Sexta  edición ",
      "Cuarta edición",
      "Primera edicion",
      "Quinta edición"
    ]
  }

];
